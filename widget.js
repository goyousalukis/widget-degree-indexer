...
cpdefine("inline:net-mydomain-widget-degreeindexer", ["chilipeppr_ready", /* other dependencies here */ ], function() {
    return {
        ...
        drawCircle: function(myAngle) {
            ...
            var c_centerX = c_width / 2;
            var c_centerY = c_height / 2;
            var ctx = c.getContext("2d");
           ...
           // var minRot = 360 / minorDiv;
            ...
            ctx.arc(c_centerX,c_centerY,circleRadius,0,2*Math.PI);
            ctx.fillStyle = "red";
            ctx.lineWidth = 2;
            ctx.stroke();
            ...
            var grd = ctx.createRadialGradient(c_centerX,c_centerY,50,c_centerX,c_centerX,c_centerX);
            ...
            //This draws little circles around inside perimeter
            for (i = 0; i<360; i=i+majRot) {
                ...
                x2 = (circleRadius - majorSize-15) * Math.sin(i * toRad);
                y2 = (circleRadius - majorSize-15) * Math.cos(i * toRad);
            
            
                /*ctx.moveTo(x1+c_centerX+5, y1+c_centerY);
                ctx.arc(x1+c_centerX,y1+c_centerY,5,0,2*Math.PI);
                ctx.stroke();
                if (i==myAngle) {
                    ctx.fill();
                    //ctx.moveTo(x1+c_centerX+3, y1+c_centerY);                    
                    //ctx.fillStyle = "black";
                    //ctx.fill();
                } */
                if (i !== myAngle) {
                    ...
                    ctx.moveTo(x1+c_centerX+1, y1+c_centerY);
                    ctx.arc(x1+c_centerX,y1+c_centerY,5,0,2*Math.PI);
                    ctx.stroke();
                    ctx.lineWidth = 1;
                    ctx.fillStyle = "grey";
                ...
                ctx.moveTo(x2+c_centerX, y2+c_centerY);
                ctx.fillStyle = "black";
                ctx.font = "10px Arial";
                var textString = (i/majRot),
                    textWidth = ctx.measureText(textString).width;
                ctx.strokeText( textString, (x2+c_centerX)-(textWidth / 2), y2+c_centerY); 
            }

            ctx.fillStyle = "black";
            ctx.font = "30px Arial";
            ctx.strokeText(("Step:" + this.decM((myAngle+degreesValue)/degreesValue)), 350,400);            
            ctx.strokeText(myAngle + "°",360,430);
